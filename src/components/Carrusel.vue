    ...
    <div class="carousel-inner">
      <div v-for="item in items" :key="item.url">
        <div :class="['carousel-item', {'active' : item.activo} ]">
          ...
          <div>
            <button class="carousel-control-prev" v-if="atras <= items.length && atras != 0" type="submit" @click="carrusel(item.url), atras--">
              <span class="carousel-control-prev-icon" aria-hidden="true"></span>
              <span class="visually-hidden">Atras</span>
            </button>
            <button class="carousel-control-next" v-if="atras < items.length-1" type="submit" @click="carrusel(item.url, true), atras++">
